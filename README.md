...
| **Vue** | `pledgestack-renderer-vue` | ❌ | ✅ | ⚠️ | ✅ | SSR + hydration + SPA nav |
| **Solid** | `pledgestack-renderer-solid` | ❌ | ✅ | ⚠️ | ✅ | SSR + hydration + SPA nav |
| **Svelte** | `pledgestack-renderer-svelte` | ❌ | ✅ | ⚠️ | ✅ | SSR + hydration + SPA nav |
...
- **CI/CD** — GitHub Actions: `cargo audit`, `cargo clippy`, `cargo fmt`, a 3-OS test matrix (Linux/macOS/Windows), Vitest. (Cross-compilation to the 6 target triples below is available via `pledge build --cross-compile`, but is not run in CI.)
...
194 test files across the monorepo using Vitest (1644 tests: 1638 passing, 6 skipped — the skips are real-CLI-gated deploy tests that require a Netlify token and similar external credentials):
...
- **Client-side navigation is framework-agnostic** — all four renderers get SPA navigation (same-origin `<a>` interception, prefetch-on-hover, popstate, scroll restoration, remount-on-swap) via `installSpaNavigation` in `pledgestack-client`, and `import { navigate } from 'pledgestack-client'` works in any framework. However, the *binding* layer is still React-heavy: `Link`, `useRouter`, `pledgestack-state` hooks (`usePersistentState`, `useUrlState`), pledge islands (`<Pledge>`), and `useActionState`/`useFormStatus` are React APIs. Non-React apps use plain `<a>` elements (intercepted automatically) and `navigate()` instead.
...
- **HMR** — Provided by the PledgePack binary's dev server, or by Vite's own dev server when using `bundler: 'vite'` (the pure-JS fallback). See [docs/limitations.md](./docs/limitations.md).
...
│   ├── bundler-vite/        # Vite bundler adapter (pure-JS fallback)
...
> **Publishing:** 30 packages are public (everything above except the two VS Code extensions, which ship through the Marketplace) and are released together at one version through [Changesets](https://github.com/changesets/changesets) — see [Releasing](#releasing). The `pledgestack` CLI still bundles every sub-package via esbuild, so it alone is enough to build an app. PledgePack is installed from npm (`pledgepack@^0.4.2`, published — `latest` dist-tag) and used to build user apps — the framework itself uses esbuild.
...
3. Merging that PR publishes all 30 public packages to npm (`pnpm release`). Versions are plain semver `0.x.y` under the `latest` dist-tag; if a prerelease line is ever needed again, `pnpm changeset pre enter <tag>` re-enters prerelease mode.
