...
import { pledgeStackVitePlugin } from './index';
...
describe('vite plugin .psx/.ps load hook', () => {
  it('compiles Rust (mocked cargo), writes the wrapper and returns the expanded module', async () => {
    const dir = await mkdtemp(join(tmpdir(), 'vite-psx-'));
    try {
      const file = join(dir, 'w.psx');
      await writeFile(file, PSX);
      const built: string[] = [];
      const plugin = pledgeStackVitePlugin({ rootDir: dir } as never, {
        build: async (req) => {
  ...
  it('.ps files re-export the written wrapper; failed builds get the fallback stub', async () => {
    const dir = await mkdtemp(join(tmpdir(), 'vite-ps-'));
    try {
      const file = join(dir, 'm.ps');
      await writeFile(file, 'pub fn add(a: i32, b: i32) -> i32 { a + b }');
      const plugin = pledgeStackVitePlugin({ rootDir: dir } as never, { build: async () => false });
      const out = (await plugin.load(file)) as string;
  ...
  it('ignores non-psx ids', async () => {
    const plugin = pledgeStackVitePlugin({ rootDir: process.cwd() } as never);
