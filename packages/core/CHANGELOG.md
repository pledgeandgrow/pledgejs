...
- PSX integrations: `SeaOrmDatabase` and `MlModel` now fail at construction with an actionable error when neither a user-supplied `driver`/`executor` nor the (unshipped) native addon is available, instead of failing on first use.
